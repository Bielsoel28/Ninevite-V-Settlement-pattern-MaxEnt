...
### PARTITION 1 ###
train <- sdmdata[train_group != All_Results[1,1], ] #model training groUp
...
test <- sdmdata[train_group == All_Results[1,1], ] #model test groUp
...
if(file.exists(jar)){
  FR.maxent_1<-maxent(x=maxent_train,p=as.factor(me_pa),path=".Outputs/MaxEnt models/MaxEnt_Final/1")
}
...
### PARTITION 2 ###
train <- sdmdata[train_group != All_Results[2,1], ] 
pres_train<-train[train$pa==1,] 
abs_train<-train[train$pa==0,] 
test <- sdmdata[train_group == All_Results[2,1], ] 
...
jar <- paste(system.file(package="dismo"), "/java/maxent.jar", sep='')
if(file.exists(jar)){
  FR.maxent_2<-maxent(x=maxent_train,p=as.factor(me_pa),path=".Outputs/MaxEnt models/FR.MaxEnt_Final/2")
}
...
### PARTITION 3 ###
train <- sdmdata[train_group != All_Results[3,1], ] #model training gropu, not equal to i
pres_train<-train[train$pa==1,] #all the presence points in the training group
abs_train<-train[train$pa==0,] # all the absence poings in the training group
test <- sdmdata[train_group == All_Results[3,1], ] #model test gropu, equal to i
pres_test<-test[test$pa==1,] #all presence in test data
abs_test<-test[test$pa==0,] #all absence in test data
...
jar <- paste(system.file(package="dismo"), "/java/maxent.jar", sep='')
if(file.exists(jar)){
  FR.maxent_3<-maxent(x=maxent_train,p=as.factor(me_pa),path=".Outputs/MaxEnt models/FR.MaxEnt_Final/3")
}
...
### PARTITION 4 ###
train <- sdmdata[train_group != All_Results[4,1], ] #model training gropu, not equal to i
pres_train<-train[train$pa==1,] #all the presence points in the training group
abs_train<-train[train$pa==0,] # all the absence poings in the training group
test <- sdmdata[train_group == All_Results[4,1], ] #model test gropu, equal to i
pres_test<-test[test$pa==1,] #all presence in test data
abs_test<-test[test$pa==0,] #all absence in test data
...
jar <- paste(system.file(package="dismo"), "/java/maxent.jar", sep='')
if(file.exists(jar)){
  FR.maxent_4<-maxent(x=maxent_train,p=me_pa,path=".Outputs/MaxEnt models/FR.MaxEnt_Final/4")
}
...
# VARIABLES INFLUENCE 
tiff("Outputs/Graphics/Individual graphics/FR.maxent_1.tiff", width = 6*300, height = 4*300, res = 300) # Width and height in pixels
plot(FR.maxent_1)
dev.off()

tiff("Outputs/Graphics/Individual graphics/FR.maxent_2.tiff", width = 6*300, height = 4*300, res = 300) # Width and height in pixels
plot(FR.maxent_2)
dev.off()

tiff("Outputs/Graphics/Individual graphics/FR.maxent_3.tiff", width = 6*300, height = 4*300, res = 300) # Width and height in pixels
plot(FR.maxent_3)
dev.off()

tiff("Outputs/Graphics/Individual graphics/FR.maxent_4.tiff", width = 6*300, height = 4*300, res = 300) # Width and height in pixels
plot(FR.maxent_4)
dev.off()

# VARIABLES RESPONSE

tiff("Outputs/Graphics/Individual graphics/FR.maxent_1_response.tiff", width = 10*300, height = 4*300, res = 300) # Width and height in pixels
response(FR.maxent_1, expand = 0.5, cex = 0.1)
dev.off()

tiff("Outputs/Graphics/Individual graphics/FR.maxent_2_response.tiff", width = 10*300, height = 4*300, res = 300) # Width and height in pixels
response(FR.maxent_2, expand = 0.5)
dev.off()

tiff("Outputs/Graphics/Individual graphics/FR.maxent_3_response.tiff", width = 10*300, height = 4*300, res = 300) # Width and height in pixels
response(FR.maxent_3, expand = 0.5)
dev.off()

tiff("Outputs/Graphics/Individual graphics/FR.maxent_4_response.tiff", width = 10*300, height = 4*300, res = 300) # Width and height in pixels
response(FR.maxent_4, expand = 0.5)
dev.off()
